...
def create_gabled_L_roof(base_obj, height, idx, exterior_coords, round_edges=False):
    ...
    txt_path = f"/tmp/input_gabled_l_{idx}.txt"
    out_mesh_path = f"/tmp/gabled_l_{idx}.ply"
    export_polygon_to_txt(base_obj, txt_path)

    base_extrude_height = height
    stdout, stderr, code = run_executable(CPP_PATH, [txt_path, out_mesh_path, "2000.0"])

    if code != 0:
        print("External C++ process failed. Skipping gabled-L roof generation.")
        ...
        try:
            ...
            print("Failed importing gabled-L geometry.")
    try:
